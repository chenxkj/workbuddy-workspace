...
### 评分流程（双评制·v5.11更新·方案A）

> **盲评规范**：双评分必须在新会话执行，不携带写稿上下文。

1. **写稿【会话1】** → `laochen-script-writing` 输出逐字稿 → 【结束会话1】
2. **双评分【会话2·新】** →
   - `laochen-script-scoring`：百分制（合规优先）·盲评（不参考写稿上下文）
...
> **核心变更**：写稿、评分、复盘必须在不同会话执行，避免自护偏差。

```
[会话1] 找选题 → 写稿（输出逐字稿文件） → 【结束会话1】

[会话2·新] 双评分（盲评·不携带写稿上下文） → 预测 → 【结束会话2】

拍摄 → 发布

[会话3·新] 复盘（数据先行·先读数据→再读稿子） → 【结束会话3】
```
...
| 会话 | 操作 | 禁止 |
|:----:|:-----|:-----|
| 会话1 | 找选题 + 写稿 | 禁止在同一会话评分 |
| 会话2（新） | 读逐字稿文件 → 双评分（盲评） | 禁止读取会话1的对话历史 |
| 会话3（新） | 先读数据文件 → 再读预测 → 最后读稿子 | 禁止先读稿子解释数据 |
...
5. **会话隔离（方案A）**：写稿、评分、复盘必须在不同会话执行，禁止同一会话写稿+评分/复盘
